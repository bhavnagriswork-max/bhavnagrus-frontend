...
export class ApiService {
  ...
  private isLocal = this.baseHost === 'localhost' || 
                    this.baseHost === '127.0.0.1' || 
                    this.baseHost.startsWith('192.168.') || 
                    this.baseHost.startsWith('10.') || 
                    this.baseHost.startsWith('172.') || 
                    this.baseHost.includes('.local');
  private apiUrl = this.isLocal ? `http://${this.baseHost}:5000/api` : `https://bhavnagris-backend.onrender.com/api`;
  private mediaUrl = this.isLocal ? `http://${this.baseHost}:5000` : `https://bhavnagris-backend.onrender.com`;
